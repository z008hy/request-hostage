...
module.exports = __webpack_require__("4816");
...
var constant = __webpack_require__("b3af");

// EXTERNAL MODULE: ./node_modules/@babel/runtime/helpers/esm/defineProperty.js
var defineProperty = __webpack_require__("ade3");

// CONCATENATED MODULE: ./node_modules/ajax-hook/src/xhr-hook.js
/*
 * author: wendux
...
// CONCATENATED MODULE: ./node_modules/ajax-hook/src/xhr-proxy.js
/*
 * author: wendux
 * email: 824783146@qq.com
 * source code: https://github.com/wendux/Ajax-hook
...
// CONCATENATED MODULE: ./node_modules/ajax-hook/index.js
/*
 * author: wendux
 * email: 824783146@qq.com
 * source code: https://github.com/wendux/Ajax-hook
...
// CONCATENATED MODULE: ./src/utils/route-parser.ts
...
var route_parser_matchPath = function matchPath(url, userFilterPath) {
  var _URI$parse = URI_default.a.parse(url),
      hostname = _URI$parse.hostname,
      path = _URI$parse.path;

  var _URI$parse2 = URI_default.a.parse(userFilterPath),
      filterHost = _URI$parse2.hostname,
      filterPath = _URI$parse2.path; // 命中


  return !!(hostname === filterHost && path !== null && path !== void 0 && path.includes(filterPath || ''));
};

var route_parser_resolvePath = function resolvePath(url, routing) {
  var _URI$parse3 = URI_default.a.parse(url),
      path = _URI$parse3.path;

  var uriWithProtocol = URI_default()("".concat(routing.redirectProtocol, "://").concat(routing.redirect));
  return "".concat(uriWithProtocol.toString(), "/").concat(path).split('/').filter(function (it) {
    return it;
  }).join('/');
};

var routeParser = function routeParser(url, routing) {
  var matchRoute = routing.find(function (routing) {
    return route_parser_matchPath(url, "".concat(routing.routeProtocol, "://").concat(routing.route));
  });
  if (!matchRoute) return '';
  return route_parser_resolvePath(url, matchRoute) || '';
};
var route_parser_equalPath = function equalPath(a, b) {
  return URI_default()(a).equals(b);
};
// CONCATENATED MODULE: ./src/utils/xhr-robber.ts
...
var xhr_robber_xhrRobber = function xhrRobber(status) {
  return function (routines, ignorePaths) {
    unProxy();
    xhr_proxy_proxy({
      onRequest: function onRequest(config, handler) {
        if (!status) return handler.next(config);
        var url = config.url;
        if (ignorePaths.some(function (itm) {
          return route_parser_equalPath(itm, url);
        })) return handler.next(config);
        var parseResult = routeParser(url, routines);
        if (!parseResult) return handler.next(config);
        handler.next(Object(objectSpread2["a" /* default */])(Object(objectSpread2["a" /* default */])({}, config), {}, {
          url: parseResult,
          headers: Object(objectSpread2["a" /* default */])(Object(objectSpread2["a" /* default */])({}, config.headers), {}, Object(defineProperty["a" /* default */])({}, constant["f" /* ORIGINAL_HEADER */], config.url))
        }));
      },
...
// CONCATENATED MODULE: ./src/script/spy.ts
...
var spy_process = function process(config) {
  var status = config.status,
      ignore = config.ignore;
  var ignorePaths = [ignore].filter(function (item) {
    return item;
  });
  var routines = [Object(objectSpread2["a" /* default */])({}, config)].filter(function (itm) {
    return itm.route && itm.redirect;
  });
  xhr_robber_xhrRobber(status)(routines, ignorePaths);
};

document.addEventListener(constant["g" /* SPY_CUSTOM_EVENT */], function (config) {
  var messageConfig = config;
  if (messageConfig !== null && messageConfig !== void 0 && messageConfig.detail) spy_process(messageConfig === null || messageConfig === void 0 ? void 0 : messageConfig.detail);
});
